(function () {
  ...
  function getStorageRepository() {
    ...
    if (!storageApi?.createRepository) {
      throw new Error('[Group Tags Favorites] 扩展存储仓库未加载');
    }
  ...
  async function loadPromptHistory() {
    const data = await getStorageRepository().get(PROMPT_HISTORY_STORAGE_KEY);
  ...
  async function savePromptHistory(history) {
    ...
    await getStorageRepository().set({ [PROMPT_HISTORY_STORAGE_KEY]: nextHistory });
    return nextHistory;
  }

  async function loadSpecialCategoryPosition() {
    const data = await getStorageRepository().get(POSITION_STORAGE_KEY);
  ...
  async function saveSpecialCategoryPosition(position) {
    ...
    await getStorageRepository().set({ [POSITION_STORAGE_KEY]: nextPosition });
    return nextPosition;
  }
