...
export class LocalSyncService {
    ...
    async safeWriteFile(filePath, newContent, lastKnownModifiedTime = null) {
        ...
        try {
            ...
            if (this.onSyncSuccess) {
                const count = this._countEntries(fileName, content);
    ...
    async scanAndPullChanges(options = { silent: false }) {
        ...
        try {
            ...
            // 【核心修正】：显式扫描被屏蔽的 _nai_sync 目录，获取最新的配置实体以便统计
            try {
                const syncDirHandle = await this.boundDirHandle.getDirectoryHandle('_nai_sync', { create: false });
                const syncLocal = await this.scanLocalDir(syncDirHandle, '_nai_sync', []);
                for (const [name, meta] of syncLocal.jsonFiles) {
                    local.jsonFiles.set(name, meta);
                }
            } catch (e) { /* 目录不存在则跳过 */ }
        ...
        const stats = {
            wildcards: local.files.size,
            favorites: this._countEntries('favorites.json', local.jsonFiles.get('favorites.json')?.content),
            grouptags: this._countEntries('group_tags.json', local.jsonFiles.get('group_tags.json')?.content),
            userdict: this._countEntries('user_dict.csv', local.jsonFiles.get('user_dict.csv')?.content)
        };
