...
export class LocalSyncService {
    ...
    async captureFileState(filePath) {
        if (!this.boundDirHandle) return { exists: false, content: '' };
        try {
            ...
            if (isNotFoundError(error)) return { exists: false, content: '' };
            throw error;
        }
    }

    async restoreFileState(filePath, snapshot) {
        if (snapshot?.exists) {
            return this.safeWriteFile(filePath, snapshot.content || '');
        }
        return this.deleteFile(filePath);
    }

    async directoryExists(folderPath) {
    ...
    setStorageRepository(storageRepository) {
        if (!storageRepository?.get || !storageRepository?.set || !storageRepository?.remove) {
            throw new TypeError('[LocalSyncService] 缺少完整的扩展存储仓库');
        }
        this.storageRepository = storageRepository;
    }

    getStorageRepository() {
        if (this.storageRepository) return this.storageRepository;
        const storageApi = globalThis.NaiAioStorage;
        if (!storageApi?.createRepository) {
            throw new Error('[LocalSyncService] 扩展存储仓库未加载');
        }
        this.storageRepository = storageApi.createRepository(chrome.storage.local, { logger: console });
    ...
    async safeWriteFile(filePath, newContent, lastKnownModifiedTime = null) {
        ...
        try {
            ...
            if (resId) {
                await this.getStorageRepository().set({
    ...
    async deleteFile(filePath) {
        ...
        try {
            ...
            // 目标已经不存在时，删除的最终状态仍然满足预期，应视为幂等成功。
            if (isNotFoundError(e)) return true;
            console.warn(`[LocalSyncService] 删除本地文件失败: ${filePath}`, e);
            return false;
        }
    }
    ...
    async deleteFolder(folderPath) {
        ...
        try {
            ...
            if (isNotFoundError(e)) return true;
            console.warn(`[LocalSyncService] 删除本地目录失败: ${folderPath}`, e);
            return false;
        }
    }
    ...
    async moveFile(oldPath, newPath, newContent) {
        if (!this.boundDirHandle) return false;
        // File System Access API 缺乏原子 rename，因此先保存目标状态；
        // 写入或删除源文件失败时恢复目标，避免留下半次移动或覆盖旧目标。
        let destinationSnapshot;
        try {
            destinationSnapshot = await this.captureFileState(newPath);
        } catch (error) {
            console.error(`[LocalSyncService] 无法读取移动目标状态: ${newPath}`, error);
            return false;
        }

        const rollbackDestination = async () => {
            const restored = await this.restoreFileState(newPath, destinationSnapshot);
            if (!restored) {
                console.error(`[LocalSyncService] 移动失败后无法恢复目标文件: ${newPath}`);
            }
            return restored;
        };
        ...
        const deleteSuccess = await this.deleteFile(oldPath);
    ...
    /**
     * 创建本地目录。空文件夹也需要真实落盘，否则下一次扫描会把它当作不存在。
     */
    async createFolder(folderPath) {
        ...
        try {
            ...
            console.error(`[LocalSyncService] 创建本地目录失败: ${folderPath}`, error);
            return false;
        }
    }

    /**
     * 使用“先完整写入新目录，再删除旧目录”的方式移动文件夹。
     * 任一写入失败时保留旧目录，避免形成不可恢复的半次移动。
     */
    async moveFolder(oldPath, newPath, wildcardEntries = {}, wildcardFolders = []) {
        if (!this.boundDirHandle) return false;
        const oldPrefix = `${oldPath}/`;
        const newPrefix = `${newPath}/`;

        // 不执行隐式目录合并。目标已存在时无法在低内存成本下可靠回滚其任意文件，
        // 因此直接拒绝移动，让用户先显式处理冲突目录。
        try {
            if (await this.directoryExists(newPath)) {
                console.warn(`[LocalSyncService] 移动目标目录已存在: ${newPath}`);
                return false;
            }
        } catch (error) {
            console.error(`[LocalSyncService] 无法检查移动目标目录: ${newPath}`, error);
            return false;
        }

        let targetCreated = false;
        const rollbackTarget = async () => {
            let shouldRollback = targetCreated;
            if (!shouldRollback) {
                try { shouldRollback = await this.directoryExists(newPath); } catch (_) { }
            }
            if (!shouldRollback) return true;
            const rolledBack = await this.deleteFolder(newPath);
            if (!rolledBack) {
                console.error(`[LocalSyncService] 移动失败后无法清理目标目录: ${newPath}`);
            }
            return rolledBack;
        };

        try {
            // 先重建所有空子目录；只靠文件写入无法保留空目录结构。
            const targetFolders = new Set([newPath]);
            for (const folderPath of wildcardFolders || []) {
                if (folderPath !== oldPath && !folderPath.startsWith(oldPrefix)) continue;
                const targetFolder = folderPath === oldPath
                    ? newPath
                    : `${newPrefix}${folderPath.slice(oldPrefix.length)}`;
                targetFolders.add(targetFolder);
            }
            for (const targetFolder of targetFolders) {
                if (!await this.createFolder(targetFolder)) {
                    throw new Error(`无法创建目标目录 ${targetFolder}`);
                }
                if (targetFolder === newPath) targetCreated = true;
            }

            for (const [key, content] of Object.entries(wildcardEntries || {})) {
                if (!key.startsWith(oldPrefix)) continue;
                const targetKey = `${newPrefix}${key.slice(oldPrefix.length)}`;
                if (!await this.safeWriteFile(`${targetKey}.txt`, content)) {
                    throw new Error(`移动文件夹写入失败: ${key} -> ${targetKey}`);
                }
            }

            if (!await this.deleteFolder(oldPath)) {
                throw new Error(`无法删除源目录 ${oldPath}`);
            }
            return true;
        } catch (error) {
            console.error(`[LocalSyncService] 移动文件夹失败: ${oldPath} -> ${newPath}`, error);
    ...
    async pushAllWildcards() {
        ...
        try {
            const data = await this.getStorageRepository().get(['wildcards']);
    ...
    async scanAndPullChanges(options = { silent: false, initialBind: false }) {
        ...
        try {
            ...
            const storageData = await this.getStorageRepository().get([
            ...
            if (Object.keys(updates).length > 0) {
                await this.getStorageRepository().set(updates);
            ...
            for (const task of pushTasks) {
                if (task.fileName === 'user_dict.csv') {
                    ...
                    if (existingMeta && existingMeta.content.trim() === newContent.trim()) {
                        ...
                        // 只拉齐浏览器端时间戳，避免下次再比
                        await this.getStorageRepository().set({ [`${task.key}_lastModified`]: existingMeta.lastModified });
                        continue;
                    }

                    await this.safeWriteFile(`_nai_sync/${task.fileName}`, newContent);
                    await this.getStorageRepository().set({ [`${task.key}_lastModified`]: Date.now() });
                    continue;
                }
                ...
                if (existingJsonMeta) {
                    ...
                    if (normalize(existingJsonMeta.content) === normalize(dataStr)) {
                        console.log(`[LocalSyncService] Push 跳过 ${task.fileName}：内容无变化`);
                        await this.getStorageRepository().set({ [`${task.key}_lastModified`]: existingJsonMeta.lastModified });
                        continue;
                    }
                }
