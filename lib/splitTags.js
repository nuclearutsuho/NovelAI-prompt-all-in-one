export default (tags, autoBreakBeforeWrap = false, autoBreakAfterWrap = false) => {
    ...
    for (let i = 0; i < length; i++) {
        ...
        // Detect Dynamic Block ||
        if (char === '|' && tags[i + 1] === '|') {
            if (!inDynamic && startBracketChar === '') {
                if (temp.trim() !== "") result.push(temp.trim());
                
                // Look ahead for config: ||min-max$$sep$$
                let header = '||';
                let j = i + 2;
                // Simple version: just match until the first space or tag-like char or comma
                // More robust: match the pattern ||(\d+-\d+)?(\$\$.*?\$\$)?
                let configMatch = tags.substring(j).match(/^(\d+-\d+)?(\$\$.*?\$\$)?/);
                if (configMatch && configMatch[0]) {
                    header += configMatch[0];
                    j += configMatch[0].length;
                }
                ...
                i = j - 1;
                continue;
            } else if (inDynamic && startBracketChar === '') {
                if (temp.trim() !== "") result.push(temp.trim());
                result.push('||');
                inDynamic = false;
                temp = '';
        ...
        // Detect single | separator inside dynamic blocks
        if (inDynamic && char === '|' && tags[i + 1] !== '|' && startBracketChar === '') {
             if (temp.trim() !== "") result.push(temp.trim());
             result.push('|');
             temp = '';
             continue;
        ...
        if (char === ':' && tags[i + 1] === ':') {
            if (!inWeightBlock && startBracketChar === '' && !inDynamic) {
                inWeightBlock = true;
                temp += '::';
                i++;
                continue;
                ...
                // CRITICAL: We just closed a ::...:: block.
                // We always keep it as one pill now.
                // Splitting into Rail mode is now a manual action via the UI.
        ...
        if (char === "\n") {
            if (startBracketChar === '' && !inWeightBlock) {
                // Modified: Allowed even if inDynamic
                if (temp.trim() !== "") {
                    result.push(temp.trim())
                }
                result.push("\n")
                ...
                endBracketChar = ''
                inWeightBlock = false;
                // inDynamic remains true if it was true
                temp = ''
            } else {
                // 前面有括号或在权重块中
                temp += ' '
