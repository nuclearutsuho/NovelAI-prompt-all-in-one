    ...
    for (let i = 0; i < length; i++) {
        ...
        if (char === ':' && tags[i + 1] === ':') {
            if (!inWeightBlock && startBracketChar === '') {
                ...
                // Decide right now if we should keep it as one pill or split it.
                const content = temp.trim();
                const weightMatch = content.match(/^([-?\d\.]+)::(.*?)\s*::$/);
                if (weightMatch && weightMatch[2].includes(',')) {
                    // It has commas! Split it into [Header, Tags..., Footer]
                    const weightPrefix = weightMatch[1] + '::';
                    const groupContent = weightMatch[2];
                    const closingMarker = ' ::';

                    result.push(weightPrefix);
                    // Split content by comma and push as individual tags
                    // (We recursion-ish this by just splitting and pushing)
                    groupContent.split(',').forEach(part => {
                        const trimmed = part.trim();
                        if (trimmed) result.push(trimmed);
                    });
                    result.push(closingMarker);
                } else {
                    // No commas or simple tag, keep as one Pill
                    result.push(content);
                }
