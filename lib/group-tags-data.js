(function () {
  ...
  function getStorageRepository() {
    ...
    if (!storageApi?.createRepository) {
      throw new Error('[Group Tags Data] 扩展存储仓库未加载');
    }
    storageRepository = storageApi.createRepository(chrome.storage.local, { logger: console });
  ...
  async function loadStoredDictionaryOverlay() {
    ...
    const stored = await getStorageRepository().get(DICTIONARY_STORAGE_KEY);
  ...
  async function saveDictionaryOverlayData(overlay, newEntries) {
    ...
    await getStorageRepository().set({
  ...
  async function syncTranslationsToDictionary(groupTagsData) {
    if (!groupTagsData?.categories) return false;
    ...
    groupTagsData.categories.forEach(category => {
      (category.groups || []).forEach(group => {
        (group.tags || []).forEach(tag => {
          if (isSentenceGroupItem(tag)) return; // skip sentences
          const canonicalTag = toCanonicalTagKey(tag.en);
          const zhCN = sanitizeText(tag.zh);
          if (!canonicalTag || !zhCN) return;

          if (baseMap.has(canonicalTag)) {
            const baseEntry = baseMap.get(canonicalTag);
            const existingOverlay = overlay[canonicalTag] || {};
            if (checkFieldChange(baseEntry.zhCN, zhCN)) {
              if (existingOverlay.zhCN !== zhCN) {
                existingOverlay.zhCN = zhCN;
                delete existingOverlay._deleted;
                existingOverlay._lastModified = now;
                overlay[canonicalTag] = existingOverlay;
                hasChanges = true;
              }
            }
          } else {
            const existingIndex = newEntries.findIndex(entry => toCanonicalTagKey(entry.tag) === canonicalTag);
            if (existingIndex >= 0) {
              const baseEntry = newEntries[existingIndex];
              if (checkFieldChange(baseEntry.zhCN, zhCN)) {
                baseEntry.zhCN = zhCN;
                baseEntry._lastModified = now;
                hasChanges = true;
              }
            } else {
              newEntries.push({
                tag: canonicalTag,
                color: 0, count: 0, aliases: '',
                zhCN: zhCN,
                _new: true,
                _lastModified: now
              });
  ...
  async function migrateStoredGroupTagsData() {
    ...
    const stored = await getStorageRepository().get(GROUP_TAGS_STORAGE_KEY);
