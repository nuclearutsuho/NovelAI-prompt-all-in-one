...
export default class TagEditor {
    ...
    init() {
        ...
        window.addEventListener('message', (e) => {
            if (e.data?.type === '__UPDATE_TE_DENSITY__') {
                // 密度变化后标签会重新布局，延迟重绘分组框
                setTimeout(() => this.renderGroupBoxes(), 50);
            }
        });
    }
    ...
    renderGroupBoxes() {
        ...
        // 2. 收集所有标签 <li> 元素，按换行符分组
        const allItems = Array.from(this.listElement.querySelectorAll('.tag-item'));
        if (allItems.length === 0) return;

        const groups = [];
        let currentGroup = { items: [], dividerName: '' };

        allItems.forEach((li) => {
            if (li.classList.contains('tag-newline') || li.classList.contains('tag-divider-line')) {
                // 将换行符的 dividerName/groupColor 绑定到当前组（换行符之上的组）
                // 同时将换行符 <li> 自身也加入 items，使方框高度包含它
                const idx = parseInt(li.dataset.index, 10);
                const tag = this.tags[idx];
                currentGroup.items.push(li); // 换行符纳入当前组的高度计算
        ...
        const fullWidth = this.listElement.clientWidth - padLeft - padRight;

        let lastBoxBottom = -Infinity; // 追踪上一个方框的底部，用于防重叠
        ...
        groups.forEach((group) => {
            ...
            const vPad = 14;
            let top = minY - listRect.top + this.listElement.scrollTop - vPad;
            let height = (maxY - minY) + vPad * 2;

            // 防重叠：如果当前方框侵入上一个方框的底部，就裁剪顶部
            if (top < lastBoxBottom + 4) {
                const diff = (lastBoxBottom + 4) - top;
                top += diff;
                height -= diff;
            }
            if (height < 10) return; // 高度太小跳过

            lastBoxBottom = top + height;

            // 4. 创建 overlay div（满宽方框），用 groupColor 驱动颜色
            ...
            if (groupColor) {
                overlay.style.borderColor = groupColor;
                overlay.style.background = this._hexToRgba(groupColor, 0.05);
            }
            ...
            if (group.dividerName) {
                ...
                // 左侧色条和分组颜色联动
                if (groupColor) {
                    badge.style.borderLeftColor = groupColor;
                }
        ...
        li.innerHTML = `
            ...
            <div class="tag-capsule ${isCompHeader ? 'gh' : (isCompFooter ? 'gf' : (isDynHeader ? 'dh' : (isDynFooter ? 'df' : '')))}" style="border-color: ${info && info.color ? info.color : ''}; border-width: ${info && info.color ? '1.5px' : ''}; background: ${info && info.color ? `linear-gradient(135deg, #3b3b4f 0%, ${info.color}15 100%)` : ''}">
                ...
                <div class="tag-controls">
                    ...
                    ${!isDynHeader && !isDynFooter && !isNewline ? `<button type="button" class="tbtn tbtn-copy" title="${copyTitle}">📋</button>` : ''}
                    ${canAnnotate ? `<button type="button" class="tbtn tbtn-ai-annotate" title="${annotateTitle}">AI</button>` : ''}
                    ${canAnnotate ? `<button type="button" class="tbtn tbtn-ai-annotate-batch" title="${annotateBatchTitle}">AI+</button>` : ''}
                    ${hasWeight ? `
                    ...
                    ${!isDynHeader && !isDynFooter && !isNewline ? `<button type="button" class="tbtn tbtn-toggle" title="${toggleTitle}">${tag.disabled ? '👁️' : '🚫'}</button>` : ''}
                    ${isAiPendingFailed ? `<button type="button" class="tbtn tbtn-retry" title="${retryTitle}">↻</button>` : ''}
