...
export default class TagEditor {
    ...
    init() {
        ...
        document.addEventListener('click', (e) => {
            if (!this.container.contains(e.target) && !e.target.closest('.selection-box')) {
                this.clearSelection();
            }
        });
    }
    ...
    initSortable() {
        this.sortable = new Sortable(this.listElement, {
            ...
            onEnd: (evt) => {
                ...
                // Sortable MultiDrag uses 'oldIndices' (corrected spelling)
                const indices = evt.oldIndices && evt.oldIndices.length > 0 ? evt.oldIndices : (evt.oldIndex !== undefined ? [{index: evt.oldIndex}] : []);
                if (indices.length === 0) return;

                // Robust way to sync tags array with new DOM order
                // We filter out any temporary Sortable elements (ghosts, clones, etc.)
                const tagElements = Array.from(this.listElement.children).filter(el => {
                    return el.classList.contains('tag-item') && 
                           !el.classList.contains('sortable-ghost') && 
                           !el.classList.contains('sortable-clone');
                });

                // If the number of real elements doesn't match our tag count, 
                // it means Sortable hasn't finished cleaning up the DOM yet.
                // In this case, we use a more conservative approach if possible,
                // or wait for the next render.
                if (tagElements.length !== this.tags.length) {
                    console.warn('Sortable DOM sync mismatch: expected', this.tags.length, 'got', tagElements.length);
                    // Fallback: If it's a single drag, we can still use simple splice
                    if (indices.length === 1 && evt.newIndex !== undefined) {
                        const item = this.tags.splice(evt.oldIndex, 1)[0];
                        this.tags.splice(evt.newIndex, 0, item);
                    } else {
                        // For multi-drag mismatch, it's safer to re-render from current state 
                        // and abort this sync to prevent duplication.
                        this.render();
                        return;
                    }
                } else {
                    const newTags = tagElements.map(el => this.tags[parseInt(el.dataset.index, 10)]).filter(Boolean);
                    
                    // Final safety check: if mapping failed to produce full array, abort
                ...
                // Sync selection indices with new positions
                this.selectedIndices.clear();
                const currentElements = Array.from(this.listElement.children).filter(el => el.classList.contains('tag-item'));
    ...
    enterEditMode(li, index, clickEvent = null) {
        ...
        // Explicitly sync Sortable selection state
        Sortable.utils.select(li);
    ...
    handleMarqueeMove(e) {
        ...
        tagElements.forEach((el, index) => {
            ...
            if (isOverlapping) {
                this.selectedIndices.add(index);
                // Sync with Sortable state
                Sortable.utils.select(el);
            } else if (!e.ctrlKey && !e.shiftKey) {
                // If not in multi-select mode, deselect non-overlapping
                this.selectedIndices.delete(index);
