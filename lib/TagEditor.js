...
export const BUTTON_DEFS = {
    ...
    copy: {
        ...
        showFor: (tag, ctx) => !ctx.isCompHeader && !ctx.isCompFooter && !ctx.isDynHeader && !ctx.isDynFooter && !ctx.isNewline
    },
    annotate: {
        id: 'annotate',
        isBatch: true,
    ...
    smartGroup: {
        ...
        showFor: (tag, ctx) => ctx.canSplit || (ctx.isMultiSelect && ctx.isSelectionMember) || ctx.isCompHeader || ctx.isCompFooter
    },
    ...
    newline: {
        ...
        showFor: (tag, ctx) => !ctx.isCompHeader && true
    },
    del: {
        id: 'del',
        isBatch: true,
...
export default class TagEditor {
    ...
    render() {
        ...
        this.tags.forEach((tag, index) => {
            ...
            const li = this.createTagElement(tag, index, { isCompMember, isDynMember, freq, tagKey: key, isTrulyFooter });
    ...
    createTagElement(tag, index, { isCompMember = false, isDynMember = false, freq = 0, tagKey = '', isTrulyFooter = null } = {}) {
        ...
        capsule.addEventListener('dblclick', (e) => {
            ...
            if (isCompHeader || isCompFooter) return;
            e.stopPropagation();
    ...
    repositionToolbar(li, isRefresh = false) {
        ...
        if (posClass === 'pos-left') {
            // 如果在左侧，依靠 calc(100% - 4px) 锚定在固定的左边缘
            controls.style.left = 'auto';
            controls.style.right = 'calc(100% - 4px)';
        } else if (posClass === 'pos-top' || posClass === 'pos-bottom') {
            // 上下位置如果用 50% 也会漂移，必须锁定在首次悬停时的固定中心像素点
            controls.style.left = `${capsuleRect.width / 2}px`;
        } else {
            // 默认在右侧，使用像素锁定在当前的相对左侧偏移量
