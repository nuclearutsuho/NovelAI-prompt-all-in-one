...
export default class TagEditor {
    ...
    createTagElement(tag, index, { isCompMember = false, isDynMember = false } = {}) {
        ...
        if (this.groupTranslationMap && this.groupTranslationMap[cleanText]) {
            if (!info) info = {};
            info.zh = this.groupTranslationMap[cleanText];
        }
        ...
        const favTitle = dict.btn_fav || "Favorite";
        ...
        li.innerHTML = `
            <div class="tag-capsule ${isCompHeader ? 'gh' : (isCompFooter ? 'gf' : (isDynHeader ? 'dh' : (isDynFooter ? 'df' : '')))}" style="border-color: ${info && info.color ? info.color : ''}; border-width: ${info && info.color ? '1.5px' : ''}; background: ${info && info.color ? `linear-gradient(135deg, #3b3b4f 0%, ${info.color}15 100%)` : ''}">
                ...
                <div class="tag-controls">
                    ...
                    ` : ''}
                    ${!isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter ? `<button type="button" class="tbtn tbtn-fav ${isFav ? 'active' : ''}" title="${favTitle}">⭐</button>` : ''}
                    ...
                    ` : ''}
                    ${(blockMatch || (!isCompHeader && !isCompFooter && !isCompMember && !isNewline && !isDynHeader && !isDynFooter)) ? `<button type="button" class="tbtn tbtn-split" title="${splitTitle}">✂️</button>` : ''}
        ...
        if (this.groupColorMap && this.groupColorMap[cleanText]) {
            const gc = this.groupColorMap[cleanText];
