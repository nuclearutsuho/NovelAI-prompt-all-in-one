...
export default class TagEditor {
    ...
    renderGroupBoxes() {
        ...
        groups.forEach((group, i) => {
            ...
            // --- 核心优化：改用 offsetTop / offsetHeight ---
            // 排除换行符/分界线标记：如果组内最后一个元素是换行符，则方框高度只计算到它之前的那个标签。
            // 这样装饰框就只会圈住真正的标签内容，而不会把换行符本身也框进去。
            const firstItem = items[0];
            let lastItemForCalc = items[items.length - 1];
            
            // 如果最后一个是换行符且不止一个元素，向上找真正的标签
            if (items.length > 1 && (lastItemForCalc.classList.contains('tag-newline') || lastItemForCalc.classList.contains('tag-divider-line'))) {
                lastItemForCalc = items[items.length - 2];
            }

            const minY = firstItem.offsetTop;
            const maxY = lastItemForCalc.offsetTop + lastItemForCalc.offsetHeight;
