...
export const BUTTON_DEFS = {
    ...
    smartGroup: {
        ...
        showFor: (tag, ctx) => !ctx.isDynSeparator && !ctx.isDynHeader && !ctx.isDynFooter && !ctx.selectionHasDynStructural && (ctx.canSplit || (ctx.isMultiSelect && ctx.isSelectionMember) || ctx.isCompHeader || ctx.isCompFooter)
    },
    ...
    seqStep: {
        id: 'seqStep',
        isComplex: true,
        className: 'tbtn-seq-step',
        titleKey: 'btn_seq_step',
        svg: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 12H3"/><path d="m11 18 6-6-6-6"/><path d="M21 5v14"/></svg>',
        showFor: (tag, ctx) => !!ctx.wildcardInfo,
        render: (tag, ctx) => ctx.wildcardInfo
            ? ctx.editorInstance.renderSequentialStepControl(ctx.wildcardInfo)
            : ''
    },
    newline: {
        id: 'newline',
...
export default class TagEditor {
    ...
    setSequentialStepProgress(progress) {
        this.sequentialStepProgress = progress || {};
        if (this.listElement) this.render();
    }

    setRandomWildcardLocks(locks) {
        this.randomWildcardLocks = locks || {};
        if (this.listElement) this.render();
    }
    ...
    setSequentialCounters(counters) {
        ...
        // In-place update to preserve hover state and avoid toolbars disappearing
        this.seqSlotCounts = {};
        this.rndSlotCounts = {};
        this.tags.forEach((tag, index) => {
            const wildcardInfo = this.consumeWildcardTagInfo(tag.value);
            if (!wildcardInfo || !wildcardInfo.isSequential) return;
            
            const li = this.listElement.querySelector(`.tag-item[data-index="${index}"]`);
            if (!li) return;
            
            const badge = li.querySelector('.tag-counter-badge');
            if (badge) {
                const dict = this.options.dict || {};
                const title = common.escapeHtml(dict.seq_slot_title || 'Current slot');
                badge.title = `${title}: ${wildcardInfo.slot}`;
                badge.textContent = `[${wildcardInfo.count + 1}]`;
            }
        });
    }

    setSequentialStepSettings(settings) {
        this.sequentialStepSettings = settings || {};
        if (!this.listElement) {
            this.render();
            return;
        }
        ...
        // In-place update to preserve hover state and avoid toolbars disappearing
        const inputs = this.listElement.querySelectorAll('.seq-step-input');
        inputs.forEach(input => {
            const control = input.closest('.seq-step-control');
            const key = control?.dataset?.seqKey;
            if (key) {
                const parsed = Number.parseInt(this.sequentialStepSettings[key], 10);
                const step = Number.isFinite(parsed) && parsed > 1 ? parsed : 1;
                if (input.value !== String(step)) {
    ...
    consumeWildcardTagInfo(value) {
        const match = value.match(/^(([sS])?(\d+)?__([A-Za-z0-9_\/\.\-\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]+)__)$/);
        if (!match) return null;

        const isSequential = !!match[2]; // 有 s/S 前缀 = 顺序通配符
        const name = match[4];

        if (isSequential) {
            // 顺序通配符：独立槽位计数
            const slot = this.seqSlotCounts[name] || 0;
            this.seqSlotCounts[name] = slot + 1;
            const key = slot === 0 ? name : `${name}:${slot}`;
            const count = this.sequentialCounters[key] !== undefined ? this.sequentialCounters[key] : 0;
            const step = this.getSequentialStepValue(key);
            return { name, slot, key, count, step, isSequential: true };
        } else {
            // 随机通配符：独立槽位计数 + r: 前缀隔离键空间
            const slot = this.rndSlotCounts[name] || 0;
            this.rndSlotCounts[name] = slot + 1;
            const key = slot === 0 ? `r:${name}` : `r:${name}:${slot}`;
            const step = this.getSequentialStepValue(key);
            return { name, slot, key, count: null, step, isSequential: false };
        }
    }

    renderCounterBadge(wildcardInfo) {
        // 随机通配符不显示计数器徽章（没有顺序概念）
        if (!wildcardInfo || !wildcardInfo.isSequential) return '';
        const dict = this.options.dict || {};
        const title = common.escapeHtml(dict.seq_slot_title || 'Current slot');
        return `<span class="tag-counter-badge" title="${title}: ${wildcardInfo.slot}">[${wildcardInfo.count + 1}]</span>`;
    }

    renderCapsuleStepBadge(wildcardInfo) {
        // 步长 <= 1 时不显示进度徽章
        if (!wildcardInfo || wildcardInfo.step <= 1) return '';
        let currentStep = 1;
        if (wildcardInfo.isSequential) {
            const prog = this.sequentialStepProgress ? (this.sequentialStepProgress[wildcardInfo.name] || 0) : 0;
            currentStep = Math.min(prog + 1, wildcardInfo.step);
        } else {
            const lock = this.randomWildcardLocks ? this.randomWildcardLocks[wildcardInfo.key] : null;
            currentStep = lock ? Math.min((lock.progress || 0) + 1, wildcardInfo.step) : 1;
        }
        return `<span class="tag-step-badge" title="Step ${currentStep}/${wildcardInfo.step}">${currentStep}/${wildcardInfo.step}</span>`;
    }

    renderStepProgressBadge(wildcardInfo) {
        if (!wildcardInfo || wildcardInfo.step <= 1) return '';
        let currentStep = 1;
        if (wildcardInfo.isSequential) {
            const prog = this.sequentialStepProgress ? (this.sequentialStepProgress[wildcardInfo.name] || 0) : 0;
        ...
        return `<span class="seq-step-progress-badge" title="Step Progress">[${currentStep}/${wildcardInfo.step}]</span>`;
    }

    renderSequentialStepControl(wildcardInfo) {
        if (!wildcardInfo) return '';
        const dict = this.options.dict || {};
        const safeKey = common.escapeHtml(wildcardInfo.key);
        const title = common.escapeHtml(dict.seq_step_title || 'Advance after N successful images');
        const resetTitle = common.escapeHtml(dict.seq_step_reset || 'Reset step progress');

        // 顺序通配符：显示位置输入框（当前在通配符列表中的第几项）
        let positionHtml = '';
        if (wildcardInfo.isSequential && wildcardInfo.count !== null) {
            const posTitle = common.escapeHtml(dict.seq_position_title || 'Current position in wildcard list');
            positionHtml = `<input type="number" class="seq-position-input" min="1" step="1" value="${wildcardInfo.count + 1}" inputmode="numeric" aria-label="${posTitle}" title="${posTitle}">`;
        }

        // 步长 > 1 时显示重置按钮
        const resetBtnHtml = wildcardInfo.step > 1
            ? `<button type="button" class="seq-step-reset-btn" title="${resetTitle}"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></button>`
            : '';

        return `
            <div class="seq-step-control" data-seq-key="${safeKey}" data-wildcard-name="${common.escapeHtml(wildcardInfo.name || '')}" data-is-sequential="${wildcardInfo.isSequential ? '1' : '0'}">
                ${positionHtml}
                ${this.renderStepProgressBadge(wildcardInfo)}
                <input type="number" class="seq-step-input" min="1" step="1" value="${wildcardInfo.step}" inputmode="numeric" aria-label="${title}" title="${title}">
                <span class="seq-step-suffix">${common.escapeHtml(dict.seq_step_suffix || 'steps')}</span>
                ${resetBtnHtml}
            </div>`;
    }
    ...
    render() {
        ...
        this.seqSlotCounts = {}; // 顺序通配符槽位计数
        this.rndSlotCounts = {}; // 随机通配符槽位计数
    ...
    createTagElement(tag, index, { isCompMember = false, isDynMember = false, freq = 0, tagKey = '', isTrulyFooter = null, isOptionLeader = false } = {}) {
        ...
        if (isDynHeader) {
            ...
            if (config) {
                ...
                pickBadge = `<span class="tag-dyn-pick-badge">x${common.escapeHtml(displayCount)}</span>`;
            }
        }
        ...
        const minPlaceholder = dict.dyn_min_placeholder || "min";
        const maxPlaceholder = dict.dyn_max_placeholder || "max";
        const dividerPlaceholder = dict.divider_name_placeholder || "分组名称";
        const dividerTitle = dict.divider_name_title || "为此分组命名";
        const dividerColor = /^#[0-9a-fA-F]{6}$/.test(tag.groupColor || '') ? tag.groupColor : '#818cf8';
        ...
        const secondaryText = (tag.aiZhPending && !isNewline && !isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter)
            ...
            : ((info && info.zh && !isNewline) ? common.escapeHtml(info.zh) : (isNewline || isCompHeader || isCompFooter || isDynHeader || isDynFooter ? '' : '&nbsp;'));
        ...
        // 检查多选集合中是否包含动态组结构元素（头尾或分界符）
        let selectionHasDynStructural = false;
        if (isMultiSelect && isSelectionMember) {
            for (const si of this.selectedIndices) {
                const st = this.tags[si];
                if (!st) continue;
                const sv = st.value;
                const isH = sv.startsWith('||') && (sv !== '||' || st.isStart);
                const isF = sv === '||' && !st.isStart;
                const isSep = sv === '|';
                if (isH || isF || isSep) { selectionHasDynStructural = true; break; }
            }
        }
        ...
        const context = {
            ...
            isMultiSelect, isSelectionMember, isOptionLeader, selectionHasDynStructural,
            wildcardInfo: null, // 占位，稍后赋值
            editorInstance: this,
            titles: {
        ...
        const wildcardInfo = this.consumeWildcardTagInfo(cleanText);
        context.wildcardInfo = wildcardInfo; // 注入通配符信息供 BUTTON_DEFS.seqStep 使用
        ...
        if (isDynHeader) {
            toolbarHtml += `
                <div class="dyn-config-control">
                    <input type="text" class="dyn-config-input dyn-min" title="${common.escapeHtml(minConfigTitle)}" placeholder="${common.escapeHtml(minPlaceholder)}">
                    <span>-</span>
                    <input type="text" class="dyn-config-input dyn-max" title="${common.escapeHtml(maxConfigTitle)}" placeholder="${common.escapeHtml(maxPlaceholder)}">
                    <input type="text" class="dyn-config-input dyn-sep sep" title="${common.escapeHtml(sepConfigTitle)}" placeholder="">
                </div>`;
        } else if (isNewline) {
            toolbarHtml += `
                <input type="text" class="divider-name-input" value="${common.escapeHtml(tag.dividerName || '')}" placeholder="${common.escapeHtml(dividerPlaceholder)}" title="${common.escapeHtml(dividerTitle)}">
                <input type="color" class="divider-color-input" value="${dividerColor}" title="选择分组颜色">`;
        }
        ...
        const toolbarButtonsHtml = toolbarConfig.order.map(btnId => {
            ...
            const title = common.escapeHtml(btnDef.title || (btnDef.titleKey ? (dict[btnDef.titleKey] || btnId) : btnId));
        ...
        li.innerHTML = `
            ...
            <div class="tag-capsule ${isCompHeader ? 'gh' : (isCompFooter ? 'gf' : (isDynHeader ? 'dh' : (isDynFooter ? 'df' : (isDynSeparator ? 'ds' : ''))))}" style="border-color: ${info && info.color ? info.color : ''}; border-width: ${info && info.color ? '1.5px' : ''}; background: ${info && info.color ? `linear-gradient(135deg, #3b3b4f 0%, ${info.color}15 100%)` : ''}">
                <div class="tag-primary">
                    ...
                    <span class="tag-text">${common.escapeHtml(isDynSeparator ? '|' : displayTagName)}</span>
                    ${showDynWeightBadge ? dynWeightBadge : ''}
                    ${this.renderCounterBadge(wildcardInfo)}
                    ${this.renderCapsuleStepBadge(wildcardInfo)}
                </div>
        ...
        if (wildcardInfo) {
            const stepInput = li.querySelector('.seq-step-input');
            if (stepInput) {
                const stopAll = (e) => e.stopPropagation();
                stepInput.addEventListener('mousedown', stopAll);
                stepInput.addEventListener('pointerdown', stopAll);
                stepInput.addEventListener('mouseup', stopAll);
                stepInput.addEventListener('click', stopAll);
                stepInput.addEventListener('dblclick', stopAll);
                stepInput.addEventListener('focus', stopAll);
                stepInput.addEventListener('blur', stopAll);

                const commitStepValue = () => {
                    const parsed = Number.parseInt(stepInput.value, 10);
                    const nextValue = Number.isFinite(parsed) && parsed > 1 ? parsed : 1;
                    stepInput.value = String(nextValue);
                    if (this.getSequentialStepValue(wildcardInfo.key) === nextValue) return;
                    this.options.onSequentialStepChange?.(wildcardInfo.key, nextValue);
                };
                ...
                stepInput.addEventListener('keydown', (e) => {
                    ...
                    if (e.key === 'Enter') {
                        ...
                        e.preventDefault();
                        stepInput.value = String(this.getSequentialStepValue(wildcardInfo.key));
                        stepInput.blur();
                    }
                });
                ...
                const control = li.querySelector('.seq-step-control');
                if (control) {
                    control.addEventListener('wheel', (e) => {
                        e.preventDefault();
                        const current = Number.parseInt(stepInput.value, 10) || 1;
            ...
            // 顺序通配符位置输入框事件处理
            const posInput = li.querySelector('.seq-position-input');
            if (posInput && wildcardInfo.isSequential) {
                const stopAll = (e) => e.stopPropagation();
                posInput.addEventListener('mousedown', stopAll);
                posInput.addEventListener('pointerdown', stopAll);
                posInput.addEventListener('mouseup', stopAll);
                posInput.addEventListener('click', stopAll);
                posInput.addEventListener('dblclick', stopAll);
                posInput.addEventListener('focus', stopAll);
                posInput.addEventListener('blur', stopAll);

                const commitPositionValue = () => {
                    const parsed = Number.parseInt(posInput.value, 10);
                    // 用户输入的是 1-based 的位置，转为 0-based 的 counter 值
                    const nextPos = Number.isFinite(parsed) && parsed >= 1 ? parsed : 1;
                    posInput.value = String(nextPos);
                    const counterValue = nextPos - 1;
                    // 通过回调通知 popup 更新计数器
                    this.options.onSequentialCounterChange?.(wildcardInfo.name, counterValue);
                };
                ...
                posInput.addEventListener('keydown', (e) => {
                    ...
                    if (e.key === 'Enter') {
                        ...
                        e.preventDefault();
                        posInput.value = String((wildcardInfo.count || 0) + 1);
                        posInput.blur();
                    }
                });
                posInput.addEventListener('change', commitPositionValue);
                posInput.addEventListener('blur', commitPositionValue);
            }

            // 步长进度重置按钮事件处理（双击确认机制）
            const resetBtn = li.querySelector('.seq-step-reset-btn');
            if (resetBtn) {
                const stopAll = (e) => e.stopPropagation();
                resetBtn.addEventListener('mousedown', stopAll);
                resetBtn.addEventListener('pointerdown', stopAll);
                resetBtn.addEventListener('click', (e) => {
                    ...
                    if (resetBtn.classList.contains('confirming')) {
                        ...
                        this.options.onStepProgressReset?.(wildcardInfo.key, wildcardInfo.isSequential);
                    } else {
                        // 第一次点击：进入确认状态
    ...
    repositionToolbar(li, isRefresh = false) {
        ...
        if (rect.right > rightEdge - 5) {
            ...
            if (newRect.left < leftEdge + 5) {
                ...
                // 动态寻找最近的滚动/裁剪容器（主编辑器 或 角色列表容器）
                // 这样能真实反映由于 overflow-y: auto 导致的上下边界，避免被挡住
                let clipTop = 0;
                let clipBottom = window.innerHeight;
                const clipAncestor = li.closest('#editor-container, #char-list-container');
                if (clipAncestor) {
                    const clipRect = clipAncestor.getBoundingClientRect();
                    clipTop = clipRect.top;
                    clipBottom = clipRect.bottom;
                }
                const spaceAbove = capsuleRect.top - clipTop;
                const spaceBelow = clipBottom - capsuleRect.bottom;
        ...
        // 重置所有按键顺序
        const allControls = controls.querySelectorAll('.tbtn, .weight-control, .dyn-config-control, .divider-name-input, .seq-step-control');
        ...
        if (stickyIds.length > 0) {
            ...
            stickyIds.forEach((id, idx) => {
                ...
                const selector = btnDef.isComplex 
                    ? (id === 'dynWeight' ? '.dyn-weight-control' : (id === 'seqStep' ? '.seq-step-control' : `.${id}-control`)) 
                    : `.${btnDef.className}`;
    ...
    _showExternalLinkMenu(e, tagText) {
        ...
        if (rect.bottom > window.innerHeight) {
            menu.style.top = `${Math.max(10, e.clientY - rect.height - 5)}px`;
        }
