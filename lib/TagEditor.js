...
export const BUTTON_DEFS = {
    fav: {
        ...
        showFor: (tag, ctx) => !ctx.isAiPending && !ctx.isCompHeader && !ctx.isCompFooter && !ctx.isDynHeader && !ctx.isDynFooter && !ctx.isNewline
    },
    copy: {
        id: 'copy',
        className: 'tbtn-copy',
        ...
        showFor: (tag, ctx) => !ctx.isDynHeader && !ctx.isDynFooter && !ctx.isNewline
    },
    annotate: {
        id: 'annotate',
    ...
    dynWeight: {
        ...
        showFor: (tag, ctx) => !ctx.isAiPending && ctx.isDynMember,
        render: (tag, ctx) => `
    ...
    retranslate: {
        ...
        showFor: (tag, ctx) => (ctx.isAiPendingFailed || (tag.aiOriginal && !ctx.isAiPending)) && !ctx.isNewline && !ctx.isCompHeader && !ctx.isCompFooter && !ctx.isDynHeader && !ctx.isDynFooter
    },
    toggle: {
        id: 'toggle',
        ...
        render: (tag, ctx) => `
            <button type="button" class="tbtn tbtn-toggle" title="${ctx.titles.toggleTitle}">
                ${tag.disabled 
    ...
    link: {
        ...
        showFor: (tag, ctx) => !ctx.isNewline && !ctx.isCompHeader && !ctx.isCompFooter && !ctx.isDynHeader && !ctx.isDynFooter
    },
    newline: {
        id: 'newline',
...
export default class TagEditor {
    ...
    createTagElement(tag, index, { isCompMember = false, isDynMember = false, freq = 0, tagKey = '' } = {}) {
        ...
        const context = {
            isAiPending, isAiPendingFailed, isNewline, isCompHeader, isCompFooter, isDynHeader, isDynFooter, isDynMember,
            canAnnotate, canSplit, hasWeight, weightVal,
            isMultiSelect, isSelectionMember,
            titles: {
        ...
        const toolbarConfig = this.options.toolbarConfig || {
            order: ['fav', 'copy', 'annotate', 'weight', 'dynWeight', 'split', 'merge', 'retranslate', 'toggle', 'link', 'newline', 'del'],
            stickyIds: ['del']
        };
        ...
        const toolbarButtonsHtml = toolbarConfig.order.map(btnId => {
            ...
            // 简单的 SVG 按钮
            return `<button type="button" class="tbtn ${btnDef.className}" title="${title}">${btnDef.svg}</button>`;
        }).join('');
