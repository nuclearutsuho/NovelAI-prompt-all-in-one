...
export default class TagEditor {
    ...
    setTags(tags) {
        ...
        // 从旧内存中按顺位将换行符的分组命名移植到新的换行符对象上
        const oldNames = oldTags.filter(t => t.value === '\n').map(t => t.dividerName || '');
        if (oldNames.length > 0) {
            let idx = 0;
            newTags.forEach(t => {
                if (t.value === '\n') {
                    if (idx < oldNames.length && oldNames[idx]) {
                        t.dividerName = oldNames[idx];
                    }
                    idx++;
                }
            });
    ...
    render() {
        ...
        this.tags.forEach((tag, index) => {
            ...
            const li = this.createTagElement(tag, index, { isCompMember, isDynMember, freq, tagKey: key });
    ...
    renderGroupBoxes() {
        ...
        groups.forEach((group, i) => {
            ...
            // --- 性能核心优化：改用 offsetTop / offsetHeight ---
            // 这比 getBoundingClientRect() 快得多，因为它不需要处理视口坐标转换
            ...
            const vPad = 6; // 减小基础边距，从 14 改为 8，使整体视觉更紧凑
            let top = minY - vPad;
    ...
    createTagElement(tag, index, { isCompMember = false, isDynMember = false, freq = 0, tagKey = '' } = {}) {
        ...
        const canSplit = !isAiPending && !tag.aiOriginal && (blockMatch || (!isCompHeader && !isCompFooter && !(isCompMember || isDynMember) && !isNewline && !isDynHeader && !isDynFooter));
        ...
        const retranslateTitle = dict.btn_retranslate || "Retranslate";
        const annotateTitle = dict.btn_ai_annotate || "AI Annotate";
        const annotateBatchTitle = dict.btn_ai_annotate_batch || "Batch Annotate";
        const retryTitle = dict.btn_retry || "Retry";
        ...
        const duplicateBadge = (!isAiPending && freq > 1 && !isNewline && !isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter)
        ...
        li.innerHTML = `
            ...
            <div class="tag-capsule ${isCompHeader ? 'gh' : (isCompFooter ? 'gf' : (isDynHeader ? 'dh' : (isDynFooter ? 'df' : '')))}" style="border-color: ${info && info.color ? info.color : ''}; border-width: ${info && info.color ? '1.5px' : ''}; background: ${info && info.color ? `linear-gradient(135deg, #3b3b4f 0%, ${info.color}15 100%)` : ''}">
                ...
                <div class="tag-controls">
                    ...
                    ` : ''}
                    ${isNewline ? `<input type="text" class="divider-name-input" value="${common.escapeHtml(tag.dividerName || '')}" placeholder="分组名称" title="为此分组命名"><input type="color" class="divider-color-input" value="${tag.groupColor || '#818cf8'}" title="选择分组颜色">` : ''}
                    ${!isAiPending && !isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter && !isNewline ? `<button type="button" class="tbtn tbtn-fav" title="${favTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path><line x1="12" y1="11" x2="12" y2="17"></line><line x1="9" y1="14" x2="15" y2="14"></line></svg></button>` : ''}
                    ${!isDynHeader && !isDynFooter && !isNewline ? `<button type="button" class="tbtn tbtn-copy" title="${copyTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button>` : ''}
                    ${canAnnotate ? `<button type="button" class="tbtn tbtn-ai-annotate" title="${annotateTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"></path><path d="M5 3v4"></path><path d="M19 17v4"></path><path d="M3 5h4"></path><path d="M17 19h4"></path></svg></button>` : ''}
                    ${canAnnotate ? `<button type="button" class="tbtn tbtn-ai-annotate-batch" title="${annotateBatchTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"></path><path d="M5 3v4"></path><path d="M19 17v4"></path><path d="M3 5h4"></path><path d="M17 19h4"></path><path d="M18 10h4"></path><path d="M20 8v4"></path></svg></button>` : ''}
                    ${hasWeight ? `
                    <div class="weight-control">
                        <button type="button" class="tbtn tbtn-sub" title="${decWeightTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><line x1="5" y1="12" x2="19" y2="12"></line></svg></button>
                        <input type="text" class="weight-input ${weightVal > 1.05 ? 'pos' : (weightVal < 0.95 ? 'neg' : '')}" value="${weightVal.toFixed(1)}" title="${editWeightTitle}">
                        <button type="button" class="tbtn tbtn-add" title="${incWeightTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></button>
                    </div>
                    ` : ''}
                    ${!isAiPending && isDynMember ? `
                    <div class="weight-control dyn-weight-control" title="...">
                        <button type="button" class="tbtn tbtn-dyn-sub" title="${decDynWeightTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><line x1="5" y1="12" x2="19" y2="12"></line></svg></button>
                        <input type="text" class="dyn-weight-input ${(tag.dynWeight || 1) > 1 ? 'pos' : ((tag.dynWeight || 1) < 1 ? 'neg' : '')}" value="${tag.dynWeight || 1}" title="${editDynWeightTitle}">
                        <button type="button" class="tbtn tbtn-dyn-add" title="${incDynWeightTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></button>
                    </div>
                    ` : ''}
                    ${canSplit ? `<button type="button" class="tbtn tbtn-split" title="${splitTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><circle cx="6" cy="6" r="3"></circle><circle cx="6" cy="18" r="3"></circle><line x1="20" y1="4" x2="8.12" y2="15.88"></line><line x1="14.47" y1="14.48" x2="20" y2="20"></line><polyline points="8.12 8.12 12 12"></polyline></svg></button>` : ''}
                    ${isCompFooter ? `<button type="button" class="tbtn tbtn-merge" title="${mergeTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg></button>` : ''}
                    ${!isAiPending && tag.aiOriginal && !isNewline && !isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter ? `<button type="button" class="tbtn tbtn-retranslate" title="${retranslateTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><path d="M21 2v6h-6"></path><path d="M3 22v-6h6"></path><path d="M20.49 9A9 9 0 0 0 5 5.64L3 8"></path><path d="M3.51 15A9 9 0 0 0 19 18.36L21 16"></path></svg></button>` : ''}
                    ${!isDynHeader && !isDynFooter && !isNewline ? `<button type="button" class="tbtn tbtn-toggle" title="${toggleTitle}">${tag.disabled ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path><circle cx="12" cy="12" r="3"></circle></svg>` : `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"></path><line x1="2" y1="2" x2="22" y2="22"></line><path d="M14.12 14.12a3 3 0 1 1-4.24-4.24"></path></svg>`}</button>` : ''}
                    ${isAiPendingFailed ? `<button type="button" class="tbtn tbtn-retry" title="${retryTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"></path><path d="M21 3v5h-5"></path><path d="M21 12a9 9 0 1 1-9 9c-2.56 0-4.87-1.06-6.52-2.76L3 16"></path><path d="M3 21v-5h5"></path></svg></button>` : ''}
                    <button type="button" class="tbtn tbtn-del" title="${delTitle}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18"></path><path d="M6 6l12 12"></path></svg></button>
                </div>
            </div>
        ...
        if (!isAiPending && !isCompHeader && !isCompFooter && !isDynHeader && !isDynFooter && !isNewline) {
        ...
        if (btnAnnotate) {
            btnAnnotate.onclick = async (e) => {
                ...
                try {
                    ...
                    console.error('[TagEditor] Failed to annotate tag:', error);
                }
            };
        }

        const btnAnnotateBatch = li.querySelector('.tbtn-ai-annotate-batch');
        if (btnAnnotateBatch) {
            btnAnnotateBatch.onclick = async (e) => {
                e.preventDefault();
                e.stopPropagation();
                try {
                    await this.onAnnotateSelectedTags(this.getSelectedTags(), btnAnnotateBatch);
                } catch (error) {
                    console.error('[TagEditor] Failed to annotate selected tags:', error);
                }
            };
        }
        ...
        if (isDynMember) {
            ...
            if (dwc) {
                dwc.addEventListener('wheel', (e) => {
                    e.preventDefault();
                    this.updateDynWeight(index, e.deltaY < 0 ? 0.1 : -0.1, li);
                }, { passive: false });
            }
        }
        ...
        if (btnRetranslate) {
            btnRetranslate.onclick = async (e) => {
                ...
                try {
                    ...
                    console.error('[TagEditor] Failed to retranslate AI tag:', error);
                }
            };
        }
        ...
        const btnRetry = li.querySelector('.tbtn-retry');
        if (btnRetry) {
            btnRetry.onclick = async (e) => {
                ...
                try {
                    ...
                    console.error('[TagEditor] Failed to retry pending AI tag:', error);
                }
            };
        }
    ...
    repositionToolbar(li, isRefresh = false) {
        ...
        // --- 静态保持逻辑 ---
        // 如果是刷新触发，且工具栏已经有位置锁定，先检查它是否依然在屏幕内
        if (isRefresh && (controls.style.left || controls.style.right)) {
            ...
            // 如果当前位置依然安全（没出左右边界），则直接返回，保持不动
            if (currentRect.right < rightEdge - 2 && currentRect.left > leftEdge + 2) {
                return;
            }
        ...
        // --- 核心优化：静止锁定 (Static Locking) ---
        // 获取当前标签容器的几何数据
        const capsule = li.querySelector('.tag-capsule');
        const capsuleRect = capsule.getBoundingClientRect();
        
        // 为了实现“静止不动”，我们将位置锁定在相对于标签起始位置的像素值上
        // 这样即使标签向右长长，工具栏也会留在原地
    ...
    updateDynWeight(index, delta, li = null) {
        ...
        indices.forEach(i => {
            ...
            tag.dynWeight = Math.max(0, tag.dynWeight + delta);
        });
    ...
    updateTagVisuals(li, tag, index) {
        ...
        if (controls) {
            ...
            if (canSplit && !existingSplit) {
                ...
                btnSplit.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;vertical-align:middle;"><circle cx="6" cy="6" r="3"></circle><circle cx="6" cy="18" r="3"></circle><line x1="20" y1="4" x2="8.12" y2="15.88"></line><line x1="14.47" y1="14.48" x2="20" y2="20"></line><polyline points="8.12 8.12 12 12"></polyline></svg>';
                btnSplit.onclick = (e) => { e.preventDefault(); e.stopPropagation(); this.splitGroupTag(index); };
