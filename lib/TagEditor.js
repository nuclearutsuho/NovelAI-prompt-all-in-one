...
export default class TagEditor {
    ...
    initSortable() {
        this.sortable = new Sortable(this.listElement, {
            ...
            onMove: (evt) => {
                ...
                // 2. If it IS a weighted tag/header, forbid dropping "inside" another group

                // Case A: Cannot drop on a member (which is always inside)
                ...
                // If target is Header, inserting AFTER it puts us inside the group (bad)
                // If target is Header, inserting BEFORE it puts us outside (good)
                if (isRelHeader && evt.willInsertAfter) return false;

                // If target is Footer, inserting BEFORE it puts us inside the group (bad)
                // If target is Footer, inserting AFTER it puts us outside (good)
                if (isRelFooter && !evt.willInsertAfter) return false;
    ...
    updateTagVisuals(li, tag, index) {
        ...
        // Update Split Button (for weight tags)
        const controls = li.querySelector('.tag-controls');
        if (controls) {
            const existingSplit = controls.querySelector('.tbtn-split');

            if (blockMatch && !existingSplit) {
            ...
            } else if (!blockMatch && existingSplit) {
