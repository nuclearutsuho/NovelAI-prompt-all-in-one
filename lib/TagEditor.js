...
export default class TagEditor {
    ...
    repositionToolbar(li, isRefresh = false) {
        ...
        if (posClass === 'pos-left') {
            // 左侧：用负的 left 像素值锁定工具栏位置
            // left = -(toolbarWidth - 4px重叠) 使工具栏右边缘与 capsule 左边缘重叠 4px
            // 像素值在 hover 时计算一次后固定，tag 变宽时工具栏不会跟着漂移
            const toolbarWidth = controls.offsetWidth;
            controls.style.left = `${-(toolbarWidth - 4)}px`;
            controls.style.right = '';
        } else if (posClass === 'pos-top' || posClass === 'pos-bottom') {
            // 上下位置：锁定在首次悬停时的 capsule 中心点像素值
            controls.style.left = `${capsuleRect.width / 2}px`;
        } else {
            // 默认在右侧：像素锁定在 capsule 右边缘 - 4px 的重叠位置
