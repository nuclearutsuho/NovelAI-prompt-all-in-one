(function initNaiAioMessageProtocol(root, factory) {
  ...
  const HISTORY_ACTION_MESSAGE_TYPES = new Set([
    '__RESTORE_HISTORY__',
    '__APPEND_HISTORY_SNIPPET__'
  ]);
  ...
  function isBoundedSnapshot(value, maxSerializedLength = 2000000) {
    if (!isPlainObject(value)) return false;
    try {
      return JSON.stringify(value).length <= maxSerializedLength;
    } catch (_error) {
      return false;
    }
  }
  ...
  function isValidHistoryActionMessage(data) {
    if (!isKnownMessage(data, HISTORY_ACTION_MESSAGE_TYPES) || !isBoundedSnapshot(data.snapshot)) {
      return false;
    }
    if (data.type === '__RESTORE_HISTORY__') return true;
    return data.target === 'positive' || data.target === 'negative';
  }
