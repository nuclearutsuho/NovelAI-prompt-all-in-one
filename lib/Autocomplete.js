export default class Autocomplete {
    ...
    getTagInfo(text) {
        ...
        let match = this.dict.find(item => item.text.toLowerCase() === query);
        if (match) return match;

        // If input has spaces, try replacing with underscores (common in danbooru tags)
        if (query.includes(' ')) {
            const queryUnder = query.replace(/ /g, '_');
