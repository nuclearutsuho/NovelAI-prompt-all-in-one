export default class Autocomplete {
  constructor(options = {}) {
    ...
    if (!this.storageRepository?.get || !this.storageRepository?.set) {
      throw new TypeError('[Autocomplete] 缺少扩展存储仓库');
    }
    this.lifecycleScope = options.lifecycleScope || null;
    if (!this.lifecycleScope?.on || !this.lifecycleScope?.timeout || !this.lifecycleScope?.cancelTimeout) {
      throw new TypeError('[Autocomplete] 缺少生命周期作用域');
    }
    this.dict = [];
    this.wildcards = {};
    ...
    this.loadFavorites().catch(error => {
      console.error('[Autocomplete] 读取收藏夹失败:', error);
    });
  }

  async loadFavorites() {
    const data = await this.storageRepository.get("favorites");
  ...
  async load() {
    ...
    try {
      const data = await this.storageRepository.get([
        "wildcards",
        "wildcardFolders",
        "wildcardUsageStats",
      ]);
  ...
  scheduleWildcardUsageStatsSave() {
    if (this.wildcardUsageSaveTimer) {
      this.lifecycleScope.cancelTimeout(this.wildcardUsageSaveTimer);
    }

    this.wildcardUsageSaveTimer = this.lifecycleScope.timeout(() => {
      this.wildcardUsageSaveTimer = null;
      this.storageRepository.set({
        wildcardUsageStats: this.wildcardUsageStats,
      }).catch(error => console.error('[Autocomplete] 保存通配符使用统计失败:', error));
    }, 120);
  }
  ...
  clearHideTimer() {
    if (this.hideTimer) {
      this.lifecycleScope.cancelTimeout(this.hideTimer);
      this.hideTimer = null;
    }
  }
  ...
  scheduleHide(delay = 200) {
    this.clearHideTimer();
    this.hideTimer = this.lifecycleScope.timeout(() => {
      this.hideTimer = null;
      this.hide();
    }, delay);
  }
  ...
  initResizer() {
    ...
    // Load saved height
    this.storageRepository.get(["autocompleteHeight"]).then(data => {
      if (data.autocompleteHeight) {
        this.listEl.style.maxHeight = data.autocompleteHeight;
      }
    }).catch(error => console.error('[Autocomplete] 读取面板高度失败:', error));
    ...
    const onMouseUp = () => {
      ...
      }).catch(error => console.error('[Autocomplete] 保存面板高度失败:', error));
    };
    ...
    this.resizerEl.addEventListener("mousedown", (e) => {
      ...
      removeMouseMove();
      removeMouseUp();
      removeMouseMove = this.lifecycleScope.on(document, "mousemove", onMouseMove);
      removeMouseUp = this.lifecycleScope.on(document, "mouseup", onMouseUp);
    });
  }
    ...
    matches.forEach((item, index) => {
      ...
      if (!item.type && !item.isWildcard) {
        ...
        linkBtn.addEventListener("mousedown", (e) => {
          ...
          const registerCloseMenu = () => {
            removeCloseMenu = this.lifecycleScope.on(document, "mousedown", closeMenu);
          };
          this.lifecycleScope.timeout(registerCloseMenu, 0);
        });
